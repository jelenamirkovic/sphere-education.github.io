...
for $node (keys %nodes)
{
    my $contents = "sudo chmod a+w /usr/local/etc/emulab/rc/rc.testbed\n sudo echo '" . $cmds{$node} . "' >> /usr/local/etc/emulab/rc/rc.testbed\n sudo chmod 755 /usr/local/etc/emulab/rc/rc.testbed";
    $filename = '/tmp/script' . int(rand(1000000));
    open(my $fh, '>', $filename);
    print $fh $contents;
    my $runcmd = "scp  -o StrictHostKeyChecking=no " . $filename . " " . $node . "." . $exp . "." . $proj . ":" . $filename;
    print "$runcmd\n";
    system($runcmd);
    system("rm $filename");
    $runcmd = "ssh -o StrictHostKeyChecking=no " . $node . "." . $exp . "." . $proj .  " bash " . $filename;
    print "$runcmd\n";
    system($runcmd);
}
