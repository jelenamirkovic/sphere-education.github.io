    ...
    for dst in portstats:
      if dst in printdests:
        for port in portstats[dst]:
          if portstats[dst][port]/deststats[dst] >= portrto:
        ...
        for src in flows[dst][dport]:
          for sport in flows[dst][dport][src]:
            ...
            if flow.flags_dst2src[RST] or flow.flags_src2dst[RST]:
              saw_rst = True
            if flow.flags_dst2src[ACK_AND_DATA] or flow.flags_src2dst[ACK_AND_DATA]:
              saw_dataack = True


            state = ''
            if saw_syn and saw_dataack and not saw_fin:
              ongoing = ongoing + 1
              state='ON'
            elif saw_syn and not saw_dataack and not saw_fin and not saw_rst:
...
class FlowStorage:
  ...
  def add_flow_event(self, ts, src, sport, dst, dport, event, numbytes):
    flows = self.flows;
    direction=0;
    ip1=src
    ip2=dst
    ...
    if ip2 not in flows:
      flows[ip2] = dict()
      flows[ip2][port2] = dict()
      flows[ip2][port2][ip1] = dict()
